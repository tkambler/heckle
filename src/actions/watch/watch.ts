...
function buildPrompt(prompts: string[], diff: string, persona?: string): string {
  ...
  const codeSection = diff
    ...
    : "";
  return (
    `${intro} Be specific, cutting, and brutally funny. ` +
...
function flush(buffer: string[], state: { busy: boolean }, projectDir: string, heckleDir: string, persona?: string): void {
  if (state.busy || buffer.length === 0) return;
...
export function runWatch(options: { name: string; persona?: string }) {
  ...
  if (!existsSync(heckleDir)) {
    console.error(
      `No heckle directory found at ${heckleDir}. Run "heckle init -n ${name}" in your project first.`
    );
    process.exit(1);
  }
